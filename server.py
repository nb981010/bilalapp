...
def schedule_prayers_for_date(target_date: date):
    ...
    try:
        ...
        for key in prayer_keys:
            ...
            scheduler.add_job('server:_http_post_play', trigger=DateTrigger(run_date=scheduled_dt), args=[filename], id=job_id, misfire_grace_time=1)
            scheduled_count += 1
    except Exception as e:
...
@app.route('/api/test/schedule-past', methods=['POST'])
def api_test_schedule_past():
    """Test helper: schedule a DateTrigger in the past with misfire_grace_time=0.

    POST JSON: {"seconds_in_past": 60, "file": "azan.mp3", "id": "optional-id"}
    Returns job metadata and whether it's considered missed/skipped.
    """
    try:
        if not SCHEDULER_AVAILABLE or not scheduler:
            return jsonify({'status': 'error', 'message': 'Scheduler not available'}), 400
        data = request.get_json(silent=True) or {}
        secs = int(data.get('seconds_in_past', 60))
        fname = (data.get('file') or 'azan.mp3').strip()
        jid = data.get('id') or f"test-past-{int(time.time())}"
        try:
            tz = get_prayer_tz()
        except Exception:
            tz = None
        ...
        try:
            ...
            nrt = j.next_run_time
            now = datetime.now(tz)
            if nrt is None or (nrt and nrt <= now):
                status = 'missed_or_skipped'
            else:
                status = 'scheduled'
            nrt_str = nrt.isoformat() if nrt is not None else None
            return jsonify({'status': 'ok', 'job_id': jid, 'next_run_time': nrt_str, 'result': status})
        except Exception as e:
